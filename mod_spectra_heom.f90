...
integer,allocatable :: zero(:),fact(:),factfact(:)
complex*16,allocatable :: rho(:,:,:),rho_t0(:,:),rho_s(:,:)
...
!! System specific
integer nquant
real*8 gamma,temperature,lambda,omg_0
...
subroutine setup
  ...
  gamma=gamma*2*pi*3.d10*au2s
  omg_0=omg_0*2*pi*3.d10*au2s
  lambda=lambda*wave_to_J/au2J
!  eta=2*lambda/hbar
  ...
  allocate(nn(nquant,-1:KK,nn_tot),map_nplus(nquant,-1:KK,nn_tot),map_nneg(nquant,-1:KK,nn_tot))
  allocate(map_sum(0:LL),zero(nn_tot),fact(LL),factfact(LL))
  allocate(rho(nquant,nquant,nn_tot),rho_t0(nquant,nquant),rho_s(nquant,nquant))
  allocate(c_matsubara(-1:KK),omg_matsubara(-1:KK))
...
subroutine setup_parameters
  ...
  integer i,j,k
  real*8 omg,beta,eps
  complex*16 w0,cc,one

  one=(1.d0,0.d0)
  beta=1/(kb*temperature)
  w0=sqrt(one*omg_0**2-gamma**2)

  !JJ=1.0!*wave_to_J
  omg_matsubara(-1)=gamma+iota*w0
  omg_matsubara(0)=gamma-iota*w0
  c_matsubara(-1)=lambda*omg_0**2/(2*hbar*w0)  *(1.d0/tanh(0.5*beta*hbar*(w0-iota*gamma))+1)
  c_matsubara(0)=lambda*omg_0**2/(2*hbar*w0)  *(1.d0/tanh(0.5*beta*hbar*(w0+iota*gamma))-1)
  

  !eta*gamma/2.d0* (1.d0/tan(hbar*gamma/(2.d0*kb*temperature))-iota)
  do k=1,KK
    omg_matsubara(k)=2*k*pi*kb*temperature/hbar
    c_matsubara(k)=8*lambda*kb*temperature/hbar**2 * omg_0**2*gamma*omg_matsubara(k)/&
    ((4*omg_matsubara(k)**2*gamma**2)-(omg_0**2+omg_matsubara(k)**2)**2)
  enddo

  sum_c_over_nu=0.d0
  do k=KK+1,KK+200
    omg=2*k*pi*kb*temperature/hbar
    cc=8*lambda*kb*temperature/hbar**2 * omg_0**2*gamma*omg/((4*omg**2*gamma**2)-(omg_0**2+omg**2)**2)
  ...
  Sp=iota*w0/omg_0* (-hbar*lambda*omg_0**2/w0 +&
  (1+gamma/(iota*w0))*c_matsubara(-1) - (1-gamma/(iota*w0))*c_matsubara(0)  )
  Sq=c_matsubara(-1)+c_matsubara(0)
  Ap=hbar*lambda*omg_0
...
subroutine compute_rho_s
  implicit none
  integer nvec(nquant,-1:KK),n

  rho_s=rho(:,:,1)

  !nvec=0; nvec(1,-1)=2
  !call index(n,nvec,1)
  !rho_s=rho_s+0.5*rho(:,:,n)

  !nvec=0; nvec(1,0)=2
  !call index(n,nvec,1)
  !rho_s=rho_s+0.5*rho(:,:,n)

  !nvec=0; nvec(1,0)=2;nvec(1,-1)=2
  !call index(n,nvec,1)
  !rho_s=rho_s+0.25*rho(:,:,n)

end subroutine compute_rho_s
!-----------------------------------------------------------------  
...
subroutine write_output(i)
  ...
  do j=1,nquant
    if(flag_spectra==0)write(100,'(f15.7$)')real(rho_s(j,j))
  enddo
  write(100,*)
...
subroutine compute_deriv_n(n,rho,drho_dt)
  ...
  integer nvec(nquant,-1:KK)!,nvec_plus(nquant,KK),nvec_neg(nquant,KK)

    mat_tmp=Q_op(:,:,1)
    m=n
    call index(m,nvec,0)
    tmp=-iota*commute(Hamil_e,rho(:,:,n))/hbar 
    tmp = tmp - 2*gamma*nvec(1,-1) * rho(:,:,n)
    ...
    nplus=map_nplus(1,-1,n)
    if(nplus>0.and.nplus<=nn_tot) then
      nplus=map_nneg(1,0,nplus)
      if(nplus>0.and.nplus<=nn_tot) then
        tmp=tmp+omg_0*nvec(1,0)*rho(:,:,nplus)
      endif
    endif

    nplus=map_nplus(1,0,n)
    if(nplus>0.and.nplus<=nn_tot) then
      nplus=map_nneg(1,-1,nplus)
      if(nplus>0.and.nplus<=nn_tot) then
        tmp=tmp-omg_0*nvec(1,-1)*rho(:,:,nplus)
      endif
    endif

    nplus=map_nplus(1,0,n)
    if(nplus>0.and.nplus<=nn_tot) then
      tmp=tmp-iota/hbar * commute(mat_tmp,rho(:,:,nplus))
    endif

    nplus=map_nneg(1,0,n)
    if(nplus>0.and.nplus<=nn_tot) then
      tmp=tmp-iota/hbar * Sq * nvec(1,0)*commute(mat_tmp,rho(:,:,nplus))
    endif

    !nplus=map_nplus(1,-1,n)
    !if(nplus>0.and.nplus<=nn_tot) then
    !  tmp=tmp+1.d0/hbar * Ap/sqrt(2*Sq)*anticommute(mat_tmp,rho(:,:,nplus))
    !endif

    nplus=map_nneg(1,-1,n)
    if(nplus>0.and.nplus<=nn_tot) then
      tmp=tmp-1.d0/hbar * Ap*nvec(1,-1)*anticommute(mat_tmp,rho(:,:,nplus))
    endif

    nplus=map_nneg(1,-1,n)
    if(nplus>0.and.nplus<=nn_tot) then
      tmp=tmp+iota/hbar * Sp*nvec(1,-1)*commute(mat_tmp,rho(:,:,nplus))
    endif

    !nplus=map_nplus(1,-1,n)
    !if(nplus>0.and.nplus<=nn_tot) then
    !  tmp=tmp-iota/hbar * Sp/sqrt(2*Sq)*commute(mat_tmp,rho(:,:,nplus))
    !endif
      ...
      do k=1,KK
        ...
        if(nminus>0.and.nminus<=nn_tot) then
          if(zero(nminus)==0)then
            !! Scaled HEOM approach from JCP 130, 084105 (2009)
            tmp=tmp-iota/hbar*nvec(m,k)*c_matsubara(k)*commute(mat_tmp,rho(:,:,nminus))
          endif
        endif
      enddo
    enddo
...
subroutine index(n,nvec,iflag)
  ...
  integer,intent(inout)::nvec(nquant,-1:KK)
...
subroutine compute_nn_sum_L(L,n_beg,n_end)
  ...
  integer flag_new,nvec(nquant,-1:KK)
...
subroutine compute_map
  implicit none
  integer n,m,k
  integer nvec(nquant,-1:KK),nvec_plus(nquant,-1:KK),nvec_neg(nquant,-1:KK)
  integer nplus,nneg
...
function anticommute(mat1,mat2) result(mat3)
  complex*16,intent(in) :: mat1(:,:),mat2(:,:)
  complex*16,allocatable :: mat3(:,:)
  integer i1
